...
CODE = <<-CRYSTAL
  CRT::RadioGroup.new(screen, x: 2, y: 5,
...
CRT::Screen.open(alt_screen: true, raw_mode: true, hide_cursor: true) do |screen|
  ...
  # Widget
  rg = CRT::RadioGroup.new(screen, x: 2, y: 5, items: ["Small", "Medium", "Large"])
  screen.focus(rg)

  # Quit hint
  hint_style = CRT::Style.new(dim: true)
  CRT::Label.new(screen, x: 2, y: 9, text: "Up/Down to select | Ctrl+C to quit",
    style: hint_style)
