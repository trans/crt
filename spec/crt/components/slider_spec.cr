...
describe CRT::Slider do
  describe "construction" do
    it "is focusable by default" do
      screen = test_screen
      sb = CRT::Slider.new(screen, x: 0, y: 0)
    ...
    it "vertical: width 1, height = length" do
      screen = test_screen
      sb = CRT::Slider.new(screen, x: 0, y: 0, length: 8)
    ...
    it "clamps initial value" do
      screen = test_screen
      sb = CRT::Slider.new(screen, x: 0, y: 0, value: 2.0)
  ...
  describe "#draw" do
    describe "vertical" do
      it "thumb at top when value is 0.0" do
        screen = test_screen
        sb = CRT::Slider.new(screen, x: 0, y: 0, length: 5)
        screen.focus(sb)
        screen.draw
      ...
      it "thumb at bottom when value is 1.0" do
        screen = test_screen
        sb = CRT::Slider.new(screen, x: 0, y: 0, length: 5, value: 1.0)
        screen.focus(sb)
        screen.draw
      ...
      it "uses half-block at thumb edge" do
        screen = test_screen
        sb = CRT::Slider.new(screen, x: 0, y: 0, length: 4, value: 0.5)
        screen.focus(sb)
        screen.draw
      ...
      it "proportional thumb with thumb_size" do
        screen = test_screen
        sb = CRT::Slider.new(screen, x: 0, y: 0, length: 10,
  ...
  describe "#handle_event" do
    it "Down increases value" do
      screen = test_screen
      sb = CRT::Slider.new(screen, x: 0, y: 0, step: 0.1)
    ...
    it "Up decreases value" do
      screen = test_screen
      sb = CRT::Slider.new(screen, x: 0, y: 0, value: 0.5, step: 0.1)
    ...
    it "Home sets value to 0.0" do
      screen = test_screen
      sb = CRT::Slider.new(screen, x: 0, y: 0, value: 0.5)
    ...
    it "clamps at bounds" do
      screen = test_screen
      sb = CRT::Slider.new(screen, x: 0, y: 0, value: 0.0, step: 0.1)
      sb.handle_event(CRT::Ansi::Key.new(CRT::Ansi::Key::Code::Up))
      sb.value.should eq(0.0)
    end
    ...
    it "callback fires on change" do
      screen = test_screen
      received = nil
      sb = CRT::Slider.new(screen, x: 0, y: 0) { |v|
        received = v
      }
      sb.handle_event(CRT::Ansi::Key.new(CRT::Ansi::Key::Code::Down))
    ...
    it "callback does not fire when value unchanged" do
      screen = test_screen
      called = 0
      sb = CRT::Slider.new(screen, x: 0, y: 0, value: 0.0) { |_|
        called += 1
      }
      sb.handle_event(CRT::Ansi::Key.new(CRT::Ansi::Key::Code::Up))
      called.should eq(0)
